...
namespace PeopleApp
{
    class PeopleApp
    {
        ...
        {
            Courses courses = new Courses(); //new Courses
                    ...
                    case "list":
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                // Output associated courses for the student
                                if (student.courseCodes.Count > 0)
                                {
                                    ...
                                    foreach (string courseCode in student.courseCodes)
                                    {
                                        ...
                                        if (course != null)
                                        {
                                            ...
                                            foreach (DayOfWeek day in course.CourseSchedule.DaysOfWeek)
                                            {
                                                Console.WriteLine($"    Day: {day}");
                                            }
                                            Console.WriteLine($"    Start Time: {course.CourseSchedule.StartTime:hh:mmtt}");
                                            Console.WriteLine($"    End Time: {course.CourseSchedule.EndTime:hh:mmtt}");
                                        }
                                    }
                                }
                            }
                            ...
                            if (thisPerson.GetType() == typeof(Teacher))
                            {
                                ...
                                Console.WriteLine($"Specialty: {teacher.specialty}");
                            }
                            ++i;
                        }
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                // Prompt for course codes and add them to the courseCodes list
                do
                {
                    Console.Write("Enter a course code (or press Enter to finish): ");
                    string courseCode = Console.ReadLine();
                    if (courseCode.Length == 0)
                    {
                        break; // Stop if an empty code is entered
                    }
