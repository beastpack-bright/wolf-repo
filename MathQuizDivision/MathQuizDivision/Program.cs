...
class Program
{
    ...
    start:
        ...
        {
            nOp = rand.Next(0, 4); // Including division 

            val1 = rand.Next(0, nMaxRange) + nMaxRange;

            // Ensure the denominator is not zero for division
            val2 = nOp == 3 ? rand.Next(1, nMaxRange) : rand.Next(0, nMaxRange);

            // Modifying the question + answers to add division
            if (nOp == 0)
            {
                nAnswer = val1 + val2;
